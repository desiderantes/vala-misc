...
public class CaseSolver {
	...
	public Gee.ArrayList<Node?> find_set(Node node, Gee.ArrayList<Gee.ArrayList<Node?>> sets) {
	...
	public double solve () {
		...
		foreach (Edge e in edges) {
			Gee.ArrayList<Node?> a = find_set (e.src, sets);
			Gee.ArrayList<Node?> b = find_set (e.dst, sets);
			if (a != b) {
				solution.add (e);
				a.add_all (b);
				sets.remove (b);
...
static int main (string[] args) {
	...
	var case_solver = new CaseSolver (nodes);
	GLib.print ("%.2lf",case_solver.solve ());
	return 0;
